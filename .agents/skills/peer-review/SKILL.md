...
Get feedback on specs, plans, code changes, or design decisions from Cursor, Codex, or Codex CLI agents.
...
Accept arguments in the form `[cursor|codex|Codex] <prompt>`.
- If the first word is exactly `cursor`, `codex`, or `Codex` and is followed by non-whitespace text, use only that tool.
...
- `/peer-review Codex Review this PR for edge cases`
...
| **Codex** | `Codex -p` | Deep code review, spec review, architecture analysis |
...
1. Parse arguments.
   - Match `cursor`, `codex`, or `Codex` as the first word only if that exact word is present and followed by non-whitespace text.
   - Treat `/peer-review Codex` with no remaining prompt as an error and ask for a prompt.
...
4. Prepare the prompt. External CLIs do not inherit the current session context, so serialize everything needed for review.
   - **CRITICAL:** Always prepend the following instruction to every prompt sent to an external agent:
     `"You are being invoked as a reviewer. Provide YOUR OWN review directly. Do NOT invoke other tools, agents, or CLIs (cursor, codex, Codex) — you are the reviewer, not a dispatcher."`
   - This prevents recursive agent invocation where Codex tries to call Cursor/Codex from within its review.
...
5. Run the review:
   ...
   **Codex** (any review type):
   ```bash
   # Short prompts:
   cd "$REPO_ROOT" && Codex -p "Your prompt here" --allowedTools Bash,Read,Glob,Grep

   # Long prompts (use stdin):
   cd "$REPO_ROOT" && cat "$PROMPT_FILE" | Codex -p - --allowedTools Bash,Read,Glob,Grep
   ```
   Notes:
   - Use `Codex -p` for non-interactive execution.
   - Restrict review runs to `Bash,Read,Glob,Grep`.
   - Change into the repo root before invoking Codex.
   - Let Codex read files with its tools when needed for code review.
...
8. Present results with clear labels.
   - In conversation, use `## Cursor feedback`, `## Codex feedback`, and `## Codex feedback`.
   - For GitHub issues, post separate comments headed `## Review: Cursor`, `## Review: Codex`, and `## Review: Codex`.
...
- Cursor has no stdin mode, so use a temp file for large prompts. Codex and Codex support stdin.
- Keep the reviewer different from the authoring agent. If Codex wrote the work, review with Cursor or Codex. If Cursor wrote it, review with Codex or Codex.
